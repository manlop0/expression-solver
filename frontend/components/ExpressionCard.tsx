...
export default function ExpressionCard({ data }: { data: Data }) {
  ...
  return (
    <div className="flex max-w-xl w-fit items-center min-h-[100px] min-w-[320px] h-fit py-2 px-6 bg-secondary rounded-xl gap-4 shadow-lg">
      {data.status === 0 ? (
        <Queued />
      ) : data.status === 1 ? (
        <Spinner />
      ...
      <div className="flex flex-col w-full items-center">
        <div className=" text-xl">{`${data.value}=${
